...
class RepliesPenelitian extends BaseController
{
    ...
    public function store()
    {
        ...
        if ($status == 1) {
            ...
            $data = $this->balasan->get_detail_penelitian($this->request->getPost('user_id'));
    ...
    public function verifikasiStatus($id)
    {
        ...
        if ($status == 1) {
            $admin = $this->user->get_user_admin();
            $pimpinan = $this->user->get_user_pimpinan();
            $data = $this->balasan->get_detail_penelitian($id);
    ...
    {
        $data = [
            'title' => 'Rekapitulasi Surat Balasan Penelitian - Bidang RIDA',
            'data_balasan' => $this->balasan->get_all_penelitian(),
        ];

        $dompdf = new \Dompdf\Dompdf();
        $dompdf->loadHtml(view('backend/rekap/penelitian/balasan', $data));
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $dompdf->stream(date('d-m-Y') . "-rekap-surat-balasan-penelitian.pdf", array('Attachment' => 0));
    }
}
