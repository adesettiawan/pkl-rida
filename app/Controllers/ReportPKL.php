...
class ReportPKL extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('backend/report/pkl/index', $data);
    }
    ...
    public function add()
    {
        ...
        return view('backend/report/pkl/add', $data);
    }
    ...
    public function store()
    {
        ...
        if ($file_reports != '') {
            $file_reports->move(ROOTPATH . 'public/assets/file_reports/pkl');
        }
        ...
        if ($status == 1) {
            ...
            $data = $this->laporan->get_detail_pkl($this->request->getPost('user_id'));
        ...
        return redirect()->to('admin/laporan_pkl');
    }
    ...
    public function verifikasiStatus($id)
    {
        ...
        if ($status == 1) {
            $admin = $this->user->get_user_admin();
            $pimpinan = $this->user->get_user_pimpinan();
            $data = $this->laporan->get_detail_pkl($id);
    ...
    public function edit($id)
    {
        $data = [
            'title' => 'Laporan - PKL Bidang RIDA',
            'users' => $this->user->get_active_user(),
            'laporan' => $this->laporan->get_detail_pkl($id),
        ];
        return view('backend/report/pkl/edit', $data);
    }
    ...
    public function update($id)
    {
        ...
        $path = '../public/assets/file_reports/pkl/';
        ...
        if ($file_rpt->isValid() && !$file_rpt->hasMoved()) {
            ...
            $file_rpt->move(ROOTPATH . 'public/assets/file_reports/pkl');
        } else {
    ...
    public function delete($id)
    {
        ...
        return redirect()->to(base_url('admin/laporan_pkl'));
    }

    public function exportPDF()
    {
        $data = [
            'title' => 'Rekapitulasi Laporan PKL - Bidang RIDA',
            'data_laporan' => $this->laporan->get_all_pkl(),
        ];

        $dompdf = new \Dompdf\Dompdf();
        $dompdf->loadHtml(view('backend/rekap/pkl/laporan', $data));
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $dompdf->stream(date('d-m-Y') . "-rekap-laporan-pkl.pdf", array('Attachment' => 0));
    }
}
